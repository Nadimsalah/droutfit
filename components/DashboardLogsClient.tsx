        ...
        <div className="space-y-8 pb-20 max-w-7xl mx-auto" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="text-left">
                    <h1 className="text-4xl font-black text-white tracking-tight italic uppercase">{dict.logsPage.title}</h1>
                    <p className="text-gray-500 text-sm font-bold mt-2 uppercase tracking-widest flex items-center gap-2">
                        <span className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse" />
                        {dict.logsPage.monitor}
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        onClick={fetchLogs}
                        className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-600/20 active:scale-95"
                    >
                        {dict.logsPage.refresh}
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 p-2 bg-[#13171F]/50 backdrop-blur-md rounded-2xl border border-gray-800/50 shadow-2xl">
                ...
                <div className="flex items-center gap-2 p-1">
                    ...
                    <select className="bg-transparent text-gray-400 text-xs font-black uppercase tracking-widest outline-none cursor-pointer hover:text-white transition-colors pr-4">
                        <option>{dict.logsPage.last24Hours}</option>
                        <option>{dict.logsPage.last7Days}</option>
                        <option>{dict.logsPage.last30Days}</option>
                    </select>
                </div>
            </div>

            <div className="bg-[#13171F] rounded-3xl border border-gray-800/50 shadow-2xl overflow-hidden min-h-[500px] flex flex-col relative text-left">
                {isLoading && (
                    <div className="absolute inset-0 bg-[#13171F]/80 backdrop-blur-sm z-20 flex items-center justify-center">
                        <div className="flex flex-col items-center gap-4">
                            <Loader2 className="h-10 w-10 animate-spin text-blue-500" />
                            <p className="text-gray-500 text-xs font-black uppercase tracking-widest">{dict.logsPage.loadingTransactions}</p>
                        </div>
                    </div>
                )}
                    ...
                    <table className="w-full text-left text-sm border-separate border-spacing-0">
                        <thead>
                            <tr className="bg-[#0B0E14]">
                                ...
                                <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-800/50 text-right">{dict.logsPage.requestId}</th>
                            </tr>
                        </thead>
                                            ...
                                            <td className="px-6 py-4 border-b border-gray-800/30">
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-2 text-gray-300 text-xs font-bold">
                                                        ...
                                                        {log.created_at ? new Date(log.created_at).toLocaleDateString(locale === 'fr' ? 'fr-FR' : 'en-US') : '—'}
                                                    </div>
                                                    <span className="text-[10px] text-gray-600 font-mono ml-5">
                                                        {log.created_at ? new Date(log.created_at).toLocaleTimeString(locale === 'fr' ? 'fr-FR' : 'en-US') : '—'}
                                                    </span>
                                                </div>
                                            </td>
