...
export default function LoginClient({ dict, locale }: { dict: any, locale: string }) {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.message || dict.auth.errorSignIn || "Failed to sign in")
        } finally {
            setIsLoading(false)
        }
    }
                ...
                <div className="bg-[#13171F] border border-gray-800 p-8 rounded-2xl shadow-2xl backdrop-blur-sm">
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">{dict.auth.password}</label>
                                <Link href={`/${locale}/forgot-password`} className="text-xs font-bold text-blue-500 hover:text-blue-400">{dict.auth.forgotPassword}</Link>
                            </div>
                            <div className="relative group">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-800 text-center">
                        <p className="text-gray-400 text-sm">
                            {dict.auth.noAccount}{" "}
                            <Link href={`/${locale}/signup`} className="text-blue-500 hover:text-blue-400 font-bold">
                                {dict.auth.createOne}
                            </Link>
                        </p>
                    </div>
