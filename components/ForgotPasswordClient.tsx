...
import { requestResetAction, verifyResetOTPAction, resetPasswordAction } from "@/app/[locale]/(auth)/forgot-password/actions"

export default function ForgotPasswordClient({ dict, locale }: { dict: any, locale: string }) {
    ...
    return (
        <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div className="w-full max-w-[440px] relative z-10">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <h1 className="text-3xl font-extrabold text-white tracking-tight text-center">
                        {step === 1 ? dict.auth.forgotPasswordTitle : step === 2 ? dict.auth.verifyEmail : dict.auth.newPassword}
                    </h1>
                    <p className="text-gray-400 mt-2 text-center text-sm max-w-[300px]">
                        {step === 1 ? dict.auth.forgotPasswordDesc :
                            step === 2 ? dict.auth.otpDesc.replace('{email}', email) :
                                dict.auth.newPasswordDesc}
                    </p>
                </div>
                    ...
                    {step === 1 ? (
                        <form onSubmit={handleRequestOTP} className="space-y-5">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-400 ml-1">{dict.auth.email}</label>
                            ...
                            >
                                {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : dict.auth.sendCode}
                            </button>
                        </form>
                    ) : step === 2 ? (
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : (
                                        <>{dict.auth.verifyAndContinue} <ShieldCheck className="h-5 w-5" /></>
                                    )}
                                </button>
                        ...
                        <form onSubmit={handleResetPassword} className="space-y-5">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-400 ml-1">{dict.auth.newPassword}</label>
                                <div className="relative group/input">
                            ...
                            >
                                {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : (
                                    <>{dict.auth.updatePassword} <CheckCircle2 className="h-5 w-5" /></>
                                )}
                            </button>
                        </form>
                    )}
