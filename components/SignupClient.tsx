...
import { signupAction, requestOTPAction } from "@/app/[locale]/(auth)/signup/actions"

export default function SignupClient({ dict, locale }: { dict: any, locale: string }) {
    ...
    useEffect(() => {
        const checkSession = async () => {
            ...
            if (session) router.push(`/${locale}/dashboard`)
        }
        checkSession()
    }, [router, locale])
    ...
    const handleRequestOTP = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.message || dict.auth.errorOtp || "Failed to send verification code")
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleSignup = async (e?: React.FormEvent) => {
        ...
        if (otpCode.length < 4) {
            setError(dict.auth.errorOtpLength || "Please enter the 4-digit code")
            return
        }
            ...
            <div className="w-full max-w-md relative z-10">
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-bold text-white tracking-tight mb-2">
                        {step === 1 ? dict.auth.signupTitle : dict.auth.verifyEmail}
                    </h1>
                    <p className="text-gray-400 text-sm">
                        {step === 1 ? dict.auth.signupDesc : dict.auth.otpDesc.replace('{email}', email)}
                    </p>
                </div>
                ...
                <div className="bg-[#13171F] border border-gray-800 p-8 rounded-3xl shadow-2xl backdrop-blur-sm relative overflow-hidden">
                    {step === 1 ? (
                        <form onSubmit={handleRequestOTP} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{dict.auth.fullName}</label>
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{dict.auth.email}</label>
                                <div className="relative group">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{dict.auth.password}</label>
                                <div className="relative group">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-800 text-center">
                        <p className="text-gray-400 text-sm">
                            ...
                            <Link href={`/${locale}/login`} className="text-blue-500 hover:text-blue-400 font-bold">
                                {dict.auth.signIn}
                            </Link>
                        </p>
                    </div>
